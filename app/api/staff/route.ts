...
export async function POST(req: NextRequest) {
  ...
  if (supabase) await supabase.from('users').upsert([{ id: userId, first_name: firstName, last_name: lastName, email, app_role: inviteeRole || 'Staff', status: 'PENDING', permissions: ['VIEW_DOCUMENTS', 'MANAGE_APPOINTMENTS'], lawyer_id: id }]);
  await sendEmail(email, `You've been invited to join OROELU GODWIN AGIDI & CO Staff Portal`, `<a href="${inviteUrl}">Activate</a>`);
