...
function signCookie(value: string) {
  ...
  if (!secret) {
    ...
    return crypto.createHmac('sha256', 'dev-secret-change-me').update(value).digest('hex');
  }
  return crypto.createHmac('sha256', secret).update(value).digest('hex');
}

export function createSessionCookie(user: { id: string; appRole: string; email?: string }) {
  const payload = JSON.stringify({ id: user.id, role: user.appRole, email: user.email || '', ts: Date.now() });
  const signature = signCookie(payload);
  return `${Buffer.from(payload).toString('base64')}.${signature}`;
}

export function readSessionCookie(req: NextRequest): { id: string; role: string; email: string } | null {
...
export function setSessionCookie(response: NextResponse, user: { id: string; appRole: string; email?: string }) {
...
export function requireRole(req: NextRequest, allowed: string[]) {
  ...
  if (!session || !allowed.includes(session.role)) {
    return { allowed: false as const, response: NextResponse.json({ error: 'Access Denied' }, { status: 403 }) };
  }
...
export async function getUserEmail(userId: string): Promise<{ email: string; firstName: string; lastName: string } | null> {
...
export function pushNotification(userId: string, notif: { type: string; title: string; message: string }) {
  if (!userId) return;
...
export const fallbackBlogPosts = [
  {
    ...
    content: "The landscape of corporate law in Africa is undergoing a massive shift...",
    date: "March 25, 2026",
    readTime: "6 min read",
  ...
  },
  {
    ...
    excerpt: "Recent policy updates have altered the visa application process...",
...
export function getCaseClientShape(c: any) {
  ...
  const statusLabel = c.status === 'ACTIVE' ? 'In Progress'
